...
def main():
    ...
    df_paep = df_paep[["control_step", "phase_id", "p_contact"]].sort_values("control_step")
    ...
    # fusion is sparse (only every 4 control steps typically); keep as sparse for markers
    # also provide a forward-filled version for a continuous line if you want
    ...
    ax1 = axes[1]
    # continuous (forward-filled) for readability
    ...
    # sparse markers at actual inference points
    ...
    ax2b = ax2.twinx()
    ax2b.plot(x, df["p_contact"].to_numpy(), linewidth=1.0, alpha=0.8, label="p_contact")
    ax2b.set_ylabel("p_contact")

    # merge legends for ax2 + ax2b
    ...
    fig.suptitle(f"Episode {ep:03d} | control_fps={cfps} inference_fps={ifps} | Phase / Injection / Fz", y=0.995)

    fig.tight_layout()
    out_path = os.path.join(BASE_DIR, "viz_phase_inj_fz_episode000.png")
    fig.savefig(out_path, dpi=200)
    print(f"[OK] saved figure to: {out_path}")

    # optional: also save merged csv for debugging
    csv_path = os.path.join(BASE_DIR, "merged_phase_inj_fz_episode000.csv")
