...
def _open_fz_csv(run_dir: str, episode_idx: int):
    ...
    w = csv.writer(f)
    w.writerow(["wall_time", "step_count", "fz"])  # wall_time: time.time()
    return csv_path, f, w
...
def _extract_fz_from_obs(np_obs_dict):
    """
    从 np_obs_dict 里尽量鲁棒地取出 z 轴力（tcp_wrench 的第3维）。
    返回 float 或 None
    """
    candidate_keys = [
    ...
    # 兼容形状：
    # (6,) / (T,6) / (n_obs_steps,6) / (B,T,6) 等
    # 你这里是单 env，通常会是 (n_obs_steps, 6) 或 (T,6)
    while w.ndim > 2:
        w = w[0]
    if w.ndim == 2:
        w = w[-1]   # 取最新时刻
...
class RealRunner:
    def __init__(self,
        ...
        self.policy_name = task_name if task_name is not None else "dp_rdp"
        self._fz_run_ts = None
        self._fz_run_dir = None
        logger.info(f"[RUNNER_INIT] task_name={task_name} policy_name={self.policy_name} output_dir={output_dir}")
        rgb_keys = list()
        lowdim_keys = list()
